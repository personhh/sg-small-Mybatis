...
public class Reflector {
    ...
    private void addDefaultConstructor(Class<?> clazz) {
        ...
        for (Constructor<?> constructor : consts) {
            //判断是否是无参构造器，是的话进行进一步处理，不是的话不做处理
            if (constructor.getParameterTypes().length == 0) {
                //对于类中私有方法访问权限的检查
                if (canAccessPrivateMethods()) {
                    //检查通过则通过setAccessible方法进行处理
                    try {
                        //构造器取消java语言访问检查，不对私有变量进行格外的限制
    ...
    //检测是否能取消反射对象在使用点上执行的标准java语言访问测
    private static boolean canAccessPrivateMethods() {
        try {
            //获取一个安全管理器的类，这个类可以在对于一些不安全或敏感的操作前执行一些操作
            SecurityManager securityManager = System.getSecurityManager();
            //判断这个安全管理的类是否为空，不为空就通过方法取消油反射对象在其使用点上执行的标准Java语言访问测查。如果取消过程中发生异常就返回false。
    ...
    private void addGetMethods(Class<?> clazz) {
        //就是有一些对象的get方法会发生冲突，所以这是一个冲突get方法集合
        ...
        for (Method method : methods) {
            ...
            //通过方法名前几个字符是不是get且方法名长度大于三，来判断方法的是不是get方法，大于三的原因：get方法没有只是get，都是get加属性，且驼峰大写
