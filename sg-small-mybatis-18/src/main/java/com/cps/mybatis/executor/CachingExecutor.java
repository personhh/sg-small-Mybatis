...
import com.cps.mybatis.cache.TransactionalCacheManager;
    ...
    public <E> List<E> query(MappedStatement ms, Object parameter, ResultHandler resultHandler, RowBounds rowBounds, CacheKey key, BoundSql boundSql) throws SQLException {
        ...
        if (cache != null) {
            ...
            if (ms.isUseCache() && resultHandler == null) {
                ...
                if (list == null) {
                    list = delegate.<E>query(ms, parameter, resultHandler, rowBounds, key, boundSql);
                    // cache：缓存队列实现类，FIFO
        ...
        return delegate.<E>query(ms, parameter, resultHandler, rowBounds, key, boundSql);
    }
